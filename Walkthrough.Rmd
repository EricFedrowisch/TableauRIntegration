---
title: "Employee Attrition Exercise w/ Machine Learning"
...
The details for this assignment are taken directly from this website:
...
```{r First 10 Rows, include=TRUE}
# View first 10 rows
hr_data_raw_tbl[1:10,] %>%
...
```{r Glimpse of the data, include=TRUE}
...
```{r Initialize Java, include=TRUE}
#Recipe code to help h2o run.
...
```{r Split the data into training, validation, and tests sets, include=TRUE}
...
```{r Model predicting attrition, include=TRUE}
# Set names for h2o
y <- "Attrition"
...
```{r Run automated machine learning, include=TRUE}
...
```{r Extract leader model, include=TRUE}
...
```{r Evaluate leader model, include=TRUE}
...
```{r Get confusion table, include=TRUE}
...
```{r Run binary classificaiton to better understand the model, include=TRUE}
...
```{r Identify class of model leader, include=TRUE}
class(automl_leader)
```
...
```{r Create model type function, include=TRUE}
...
```{r Create predict_model function, include=TRUE}
...
```{r Test the predict_model function, include=TRUE}
...
```{r Run LIME on training set, include=TRUE}
...
```{r Run explain function, include=TRUE}
...
```{r Visualize Feature Importance, include=TRUE}
#plot features
plot_features(explanation) +
    labs(title = "HR Predictive Analytics: LIME Feature Importance Visualization",
         subtitle = "Hold Out (Test) Set, First 10 Cases Shown")

#Note: The graphic will likely not look right unless you have a large enough monitor. This needs to be fixed. You'll need to enlarge the graphic and fit it to your full screen, most likely. 
```
...
```{r Focus on Critical Features of Attrition, include=TRUE}
...
```{r Training as a Cause, include=TRUE}
attrition_critical_features %>%
...
```{r Overtime as a Cause, include=TRUE}
attrition_critical_features %>%
mutate(OverTime = 
case_when(
OverTime == "Yes" ~ 1,
